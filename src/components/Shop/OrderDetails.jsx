...
import { AllOrderOfShop } from "../../redux/actions/order";
...
const OrderDetails = () => {
  ...
  useEffect(() => {
    dispatch(AllOrderOfShop(seller._id));
  }, [dispatch]);
  ...
  const orderUpdateHandler = async (e) => {
    ...
    // console.log("Status being sent:", data?.status);

    const statusTosubmit = status || data?.status
    await axios
      .put(
  ...
  return (
    <div className={`py-4 min-h-screen ${styles.section}`}>
      ...
      {
        data?.status === "Processing refund" || data?.status === "Refund Success" ? (
          <select value={status || data?.status || ''}
            onChange={(e) => setStatus(e.target.value)}
            className="w-[200px] mt-2 border h-[35px] rounded-[5px]"
          >
